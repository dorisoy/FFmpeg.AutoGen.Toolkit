...
public sealed unsafe class H264VideoStreamEncoder : IDisposable
{
    ...
    {
        // From https://ffmpeg.org/doxygen/4.1/group__lavc__encdec.html:
        // 流结束情况。这需要对编解码器进行“刷新”（又称排空），因为编解码器可能会在内部缓冲多个帧或数据包以提高性能或出于必要（考虑B帧）。处理方式如下：

        //将NULL发送给avcodec_send_packet（）（解码）或avcodec_send_frame（编码）函数，而不是有效输入。这将进入排水模式。

        //在循环中调用avcodec_receive_frame（）（解码）或avcodec_receive_packet（））（编码），直到返回AVERROR_EOF。除非您忘记进入排水模式，否则功能不会返回AVERROR（EAGAIN）。
